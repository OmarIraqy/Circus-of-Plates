...
public class ShapesFactory {
    ...
    public Shapes getShape(int width,int height, String shapeName) {
        if (shapeName == "GreenPlate") {
            return new ImageObject((int) (Math.random() * (width/2)), -1 * (int) (Math.random() * height), "./images/greenplatewithbase.png",1);
        } else if (shapeName == "RedPlate") {
            return new ImageObject((int) (Math.random() *(width/2)), -1 * (int) (Math.random() * height), "./images/redplatewithbase.png",2);
